...
## Production Deployment and Prediction System

### Energy Load Prediction System

After completing the comprehensive training pipeline in this repository, the trained models can be deployed for real-time energy load predictions using a dedicated prediction system available at:

**🔗 [Predictions-Changepoint](https://github.com/aitorDiezMateo/Predictions-Changepoint.git)**

This companion repository provides a production-ready prediction API that uses the models and parameters generated by this training pipeline to make energy load forecasts.

### Relationship Between Projects

**This Repository (Building Heat Load Characterization and Clustering)**:
- **Purpose**: Complete model training, analysis, and research pipeline
- **Functionality**: Data processing, changepoint modeling, clustering analysis, CART training, and comprehensive validation
- **Output**: Generates trained models (`.pkl` files) and parameter files (`.csv` files) for deployment
- **Focus**: Research-oriented with extensive analysis, visualization, and model development

**Prediction System Repository (Predictions-Changepoint)**:
- **Purpose**: Production deployment and inference system for energy load predictions
- **Functionality**: Uses pre-trained models to make real-time or batch energy load predictions
- **Requirements**: Trained CART models and changepoint parameters from this repository
- **Focus**: Production-ready prediction pipeline with simple API for operational use

### Model Export and Deployment Workflow

**Step 1: Train Models (This Repository)**:
```bash
# Execute complete training pipeline
snakemake --cores 8

# Generated files for deployment:
# - output/data_09_cart_model.pkl (CART classification model)
# - output/data_09_Changepoint_Pars_summ_CLUST_PRED.csv (regression parameters)
```

**Step 2: Deploy for Predictions (Prediction Repository)**:
```bash
# Clone prediction system
git clone https://github.com/aitorDiezMateo/Predictions-Changepoint.git

# Copy trained models
cp output/data_09_cart_model.pkl Predictions-Changepoint/models/
cp output/data_09_Changepoint_Pars_summ_CLUST_PRED.csv Predictions-Changepoint/parameters/

# Use for predictions
python -c "
from src.predictions import predict_energy_load
import pandas as pd
features = pd.DataFrame([{
    'Temperature': 20.5, 'Solar_Irradiation': 1800, 'Holiday': 0,
    'DATE_day_year': 150, 'DATE_day_week': 3, 'DATE_weekday': True
}])
load = predict_energy_load('models/data_09_cart_model.pkl', 
                          'parameters/data_09_Changepoint_Pars_summ_CLUST_PRED.csv',
                          features, hour=14)
print(f'Predicted load: {load} kWh')
"
```

### Two-Stage Prediction Process

The prediction system implements the two-stage approach developed in this training pipeline:

1. **Daily Classification**: Uses the trained CART model to predict which cluster (operational pattern) a day belongs to based on:
   - Daily average temperature
   - Daily total solar irradiation  
   - Calendar features (day of year, day of week, holiday status)

2. **Hourly Load Prediction**: Uses cluster-specific changepoint parameters to calculate energy load:
   ```
   Load = max(minimum, intercept + slope_Temp × temperature + slope_Irrad × solar_irradiation)
   ```

### Key Features of the Prediction System

- **✅ Daily-to-Hourly Prediction**: Input daily characteristics, get hourly load predictions
- **✅ Cluster-Based Modeling**: Uses the trained clusters for improved accuracy
- **✅ Batch Processing**: Predict multiple days and hours efficiently
- **✅ Robust Error Handling**: Graceful handling of missing data and edge cases
- **✅ Production Ready**: Simple API for integration into energy management systems

### Use Cases for the Prediction System

**Energy Management Applications**:
- **Load Forecasting**: Predict building energy demand for next day/week
- **Demand Response**: Optimize energy consumption during peak periods
- **System Planning**: Size heating/cooling systems based on predicted loads
- **Cost Optimization**: Schedule energy-intensive operations during low-cost periods

**Integration Scenarios**:
- **Building Management Systems (BMS)**: Real-time load prediction for HVAC control
- **District Heating Networks**: Aggregate load forecasting for multiple buildings
- **Energy Trading**: Day-ahead market participation with load predictions
- **Smart Grid Applications**: Grid stability and demand-side management
